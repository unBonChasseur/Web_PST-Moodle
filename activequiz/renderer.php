...
class mod_activequiz_renderer extends plugin_renderer_base {
    ...
    public function render_question_form($slot, $attempt) {
        ...
        $timertext = html_writer::div(get_string('timertext', 'activequiz'), 'timertext', array('id' => 'q' . $qnum . '_questiontimetext','style' => 'display:inline-block;'));
        $timercount = html_writer::div('', 'timercount', array('id' => 'q' . $qnum . '_questiontime','style' => 'display:inline-block;'));
        ...
        if ($rtqQuestion->getTries() > 1 && !$this->rtq->is_instructor()) {
            ...
            $trytext = html_writer::div(get_string('trycount', 'activequiz', $count), 'trycount', array('id' => 'q' . $qnum . '_trycount','style' => 'display: inline-block;'));
        } else {
            $trytext = html_writer::div('', 'trycount', array('id' => 'q' . $qnum . '_trycount','style' => 'display: inline-block;'));;
        }

        // instructors don't need to save questions
        if (!$this->rtq->is_instructor()) {
            $savebtncont = html_writer::div($savebtn, 'question_save',array('style' => 'display: inline-block;'));
        } else {
            $savebtncont = '';
        }
